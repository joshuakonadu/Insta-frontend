    ...
    <div class="containerSelf h-100">
      <div v-if="userImage.imageUri" class="row align-items-center h-100">
        <div class="col-md-8 mr-0 pr-0">
          <img
            :src="`${imageBaseUrl}/${userImage.userId}/${userImage.imageUri}`"
            class="card-img-top minH"
          />
        </div>
        <div class="ml-0 pl-0 col-md-4">
          <div class="card mb-0 minH2">
            <div class="card-header headerH">
              <span
                v-if="avatar.imageB64"
                class="avatar avatar-md"
                :style="{
                  'background-image': `url(data:image/${avatar.format};base64,${avatar.imageB64})`
                }"
              ></span>
              <span
                v-else
                class="avatar avatar-md"
                :style="{ 'background-image': `url(@/assets/user.jpg)` }"
              ></span>
            </div>
            <div class="card-body">
              <p>blank</p>
              ...
              <div class="row">
                ...
                <div class="col-12">
                  <span class="smText text-muted"
                    >Vor {{ userImage.createdAt | elapsed }}</span
                  >
                </div>
              </div>
            </div>
            <div v-if="!loggedIn" class="card-footer">
              <router-link tag="a" to="/login"> Melde dich an</router-link>, um
              mit „Gefällt mir“ zu markieren oder zu kommentieren.
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  created() {
    if (!this.$store.state.user) this.$store.registerModule("user", user);
