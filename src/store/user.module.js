...
export const user = {
  ...
  actions: {
    ...
    updateAvatar({ commit }, edit) {
      userService
        .changeAvatar(edit)
        .then(avatar => commit("updateAvatar", avatar.data.avatar));
    },
    uploadImages({ commit }, data) {
      userService
        .uploadImages(data)
        .then(images => commit("uploadImagesSuccess", images.data));
    },
    getUserImage({ commit }, id) {
      userService
        .getUserImage(id)
        .then(data => commit("getUserImageSuccess", data.data));
    },
    deleteImage({ commit }, data) {
      userService
        .deleteImage({ id: data.id, fileName: data.fileName })
        .then(() => {
