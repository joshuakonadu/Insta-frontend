    ...
    <div class="row">
      <div class="col-4">
        <img
          ...
          v-bind:src="
            'data:image/' +
              $store.state.user.avatar.format +
              ';base64,' +
              $store.state.user.avatar.imageB64
          "
        ...
        <div class="avatar-statusSelf">
          ...
          >
            <label
              class="btn btn-sm btn-primary file-attachment-btn mb-1 mb-sm-0 mr-1"
              for="avatarBtn"
            >
      ...
      <div class="col-8 align-self-stretch">
        ...
        <div v-if="!lock" class="row">
          <div class="">
            <button @click="lockFalse" class="btn btn-sm btn-primary">
              Info Bearbeiten
            </button>
          </div>

          <div class="ml-3">
            <button @click="showUploadModal" class="btn btn-sm btn-secondary">
              Bilder Hochladen
            </button>
          </div>
        </div>
        <div v-else class="row">
          ...
          <div class="ml-3">
            <button @click="cancel" class="btn btn-sm btn-secondary">
              Abbrechen
            </button>
          </div>
        </div>
  ...
  methods: {
    updateAvatar(event) {
      ...
      reader.onloadend = function() {
        that.$store.dispatch("user/updateAvatar", {
